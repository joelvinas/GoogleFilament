...
### Renderer
- **[CameraRenderer.kt](file:///C:/Users/audranian/source/repos/GoogleFilament/android-filament-demo/app/src/main/java/com/example/filamentdemo/ui/samples/CameraRenderer.kt)**:
    - **Async Material Compilation**: Refactored `setupMaterialAsync` to use Kotlin Coroutines. `MaterialBuilder.build()` now runs on `Dispatchers.IO` to prevent main-thread UI hitching, while the results are applied on `Dispatchers.Main` for JNI safety. Added safety checks for lifecycle destruction during compilation.
