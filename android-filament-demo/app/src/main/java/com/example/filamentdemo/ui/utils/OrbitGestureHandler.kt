...
class OrbitGestureHandler(
    ...
    fun onTouchEvent(event: MotionEvent): Boolean {
        // Unconditional dispatch to ScaleGestureDetector first. Its scroll() call and the
        // state machine's per-frame pan micro-session (below) both run synchronously within
        // this method, never overlapping, so they can't race for the same manipulator frame.
